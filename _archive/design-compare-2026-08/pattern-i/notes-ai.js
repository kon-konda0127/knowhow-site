...
function renderNotes(notes) {
  ...
  function renderList() {
    ...
    list.innerHTML = filtered.length === 0
      ...
      : filtered.map(n => `<a href="notes/${n.id}.html" class="note-card"><h3>${n.title}</h3><p style="font-size:0.82rem;color:#425466;">${n.category}</p><div class="tags">${n.tags.map(t=>`<span class="note-tag">${t}</span>`).join('')}</div></a>`).join('');
  }
