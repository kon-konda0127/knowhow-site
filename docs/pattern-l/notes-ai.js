const AI_NOTES = [
  {
    ...
    points: [
      ...
      "図解は矢羽根/ロードマップ/フレームワーク表/バブル/滝グラフの型から自動選択",
      "配色は脇役グレー＋主役の濃紺1色、凡例なしの直接ラベルに統一",
      "アクセント色だけ任意のカラーに差し替え可能",
      "A4印刷でそのまま配布できるクオリティ"
    ]
  },
  {
    ...
    points: [
      "model-routerでCodexのsubagent（AIが別のAIに指示を出す仕組み）を拡張し、タスクごとに異なるモデルを使い分けられる",
      "重い判断・設計はOpus等の上位モデル、定型的なコード生成はDeepSeek等の軽量モデルに自動振り分け",
      "全タスクに最上位モデルを使い続けるとコスト過多になる問題を解決する",
      "コスト構造そのものを変える設計思想"
    ]
  }
];
...
function renderNotes(notes) {
  ...
  function renderList() {
    ...
    list.innerHTML = filtered.length === 0
      ...
      : filtered.map(n => `<a href="notes/${n.id}.html" class="note-card"><h3>${n.title}</h3><p style="font-size:0.82rem;color:#757575;font-family:sans-serif;">${n.category}</p><div class="tags">${n.tags.map(t=>`<span class="note-tag">${t}</span>`).join('')}</div></a>`).join('');
  }
